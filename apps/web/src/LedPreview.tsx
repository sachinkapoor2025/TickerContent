...
export function LedPreview({
  ...
  useEffect(() => {
    ...
    let raf = 0;
    const loop = (t: number) => {
      renderFrame(document, ctx, t - start, resourcesRef.current);
  ...
  return <canvas ref={ref} className={`led-canvas ${className ?? ""}`} role="img" aria-label={label} />;
}
